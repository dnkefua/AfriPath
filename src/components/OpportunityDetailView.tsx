...
export const OpportunityDetailView: React.FC<OpportunityDetailViewProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto pb-32">
        ...
        <section className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {opportunity.internationalApplicantPolicy && (
              <span className="bg-amber-950/60 border border-amber-500/20 text-amber-300 px-3 py-1 rounded-full text-xs font-mono">
                {opportunity.internationalApplicantPolicy}
              </span>
            )}
          </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="glass-panel p-6 rounded-2xl border border-white/10 text-center">
              ...
              <p className="text-lg font-mono font-bold text-amber-400 flex items-center justify-center gap-2">
                <Calendar className="w-4 h-4" />
                {opportunity.deadline}
              </p>
              <p className="text-[11px] text-slate-400 leading-relaxed mt-3 pt-3 border-t border-white/5">
                Always confirm the current deadline on the official program page before planning
                your application.
              </p>
            </div>
            ...
            {opportunity.sourceUrl && (
              <div className="glass-panel p-6 rounded-2xl border border-[#cdebd8]">
                ...
                <p className="text-xs text-[#45464d] leading-relaxed mb-4">
                  Last checked {opportunity.lastVerifiedAt || "recently"} against the program or
                  government source. Applications happen on the official site, never inside
                  AfriPath.
                </p>
                <a
      ...
      <div className="fixed bottom-0 left-0 w-full bg-[#020205]/95 border-t border-white/5 px-6 py-4 flex items-center gap-3 z-40 shadow-xl backdrop-blur-md">
        <button
          onClick={() => onToggleSave(opportunity.id)}
          aria-label={isSaved ? "Remove from saved" : "Save opportunity"}
          className={`flex items-center justify-center w-14 h-14 flex-shrink-0 rounded-xl border border-white/10 hover:border-cyan-400/50 hover:bg-white/5 transition-all ${
        ...
        <button
          ...
          disabled={isTracked}
          className={`h-14 px-4 rounded-xl border text-xs font-bold transition-all flex items-center justify-center gap-2 ${
            isTracked
              ? "border-white/10 bg-white/5 text-slate-500 cursor-not-allowed"
              : "border-cyan-400/40 bg-cyan-500/10 text-cyan-300 hover:bg-cyan-500/20"
          } ${opportunity.sourceUrl ? "flex-shrink-0" : "flex-grow"}`}
        >
